//
//  AWHOilSwift.h
//  AWHOilSwift
//
//  Created by 王恒 on 2022/10/9.
//

#import <Foundation/Foundation.h>

//! Project version number for AWHOilSwift.
FOUNDATION_EXPORT double AWHOilSwiftVersionNumber;

//! Project version string for AWHOilSwift.
FOUNDATION_EXPORT const unsigned char AWHOilSwiftVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <AWHOilSwift/PublicHeader.h>

#import <AWHOilSwift/AWHOSQuantityStatisticsDetailsModel.h>
